...
def kp_detection(db, nnet, result_dir, debug=False, evaluator=None, repeat=1,
                 isEncAttn=False, isDecAttn=False,progressBar = None,image_root=None):
    if db.split != "train":
    ...
    image_names.sort(key=lambda x: int(x[:-4]))  # 倒着数第四位'.'为分界线，按照‘.’左边的数字从小到大排序
    ...
    if progressBar is not None:
        ...
        progressBar.setMaximum(num_images)
    ...
    for ind in tqdm(range(0, num_images), ncols=67, desc="locating kps"):
        ...
        if debug:
            ...
            if not isEncAttn and not isDecAttn:
                ...
                n = 1
                # while(n<300):
                cv2.imwrite("D:\TuSimple\LSTR\detections" + "\\" + str(ind+1) + ".jpg", preds)
                # cv2.imwrite(os.path.join(os.path.basename(image_file[:-4]) + '_test.jpg'), preds)
        if progressBar is not None:
...
def testing(db, nnet, result_dir, debug=False, evaluator=None, repeat=1,
            debugEnc=False, debugDec=False, progressBar = None,image_root=None):
    return globals()[system_configs.sampling_function](db, nnet, result_dir, debug=debug, evaluator=evaluator,
                                                       repeat=repeat, isEncAttn=debugEnc, isDecAttn=debugDec,progressBar = progressBar,image_root=image_root)