...
class HungarianMatcher(nn.Module):
    ...
    def __init__(self, cost_class: float = 1,
        ...
        # """Creates the matcher
        # 执行匹配参数：输出：这是一个至少包含以下条目的dict:”“pred\u logits”：dim[batch\u size，num\u querys，num\u classes]
        # 的张量与分类logits“pred\u box”：dim[batch\u size，num\u querys，4]使用预测框坐标目标：这是一个目标列表（len（targets）=批量大小），
        # 其中，每个目标是一个dict，
        # 其中包含：“labels”：dim[num\u target\u box]的张量（其中num\u target\u box是目标中地面真值对象的数量），
        # 包含类标签“box”：dim[num\u target\u box]的张量，4]包含目标框坐标返回：
        # 一个size batch\u size的列表，包含（index\u i，索引j）其中：-索引i是所选预测（按顺序）的索引-索引j是每个批元素对应的所选目标（按顺序）的索引，
        # 它包含：len（索引i）=len（索引j）=min（num\u查询，num\u目标框）
        #
        # """
